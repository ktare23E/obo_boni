    ...
    form.put(route('update_user', form.id), {
        onSuccess: () => {
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: 'Inspector has been updated successfully.',
                confirmButtonColor: '#3085d6',
                confirmButtonText: 'OK'
            }).then(() => {
                router.visit(route('inspector'));
            });
        },
        onError: () => {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Something went wrong while updating the inspector.',
...
<template>
    <Head title="Edit Inpspector" />
    <AdminLayout :title="'Edit Inpspector'">
